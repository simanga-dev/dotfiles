...
function get_passname_from_menu() {
    ...
    passname="$(printf '%s\n' "${password_files}" | run_rofi 'Select Password' )"
    wofi_ret=${?}
...
function get_field_from_menu() {
    ...
    if [ ${FLAG_SQUASH} -eq 1 ] && [ "${field_count}" -le 1 ]; then
        ...
        printf "%s" "$(printf '%s\n' "${field_list[@]}" | run_rofi 'Select Password' )"
    fi
}
...
function do_output() {
    local -r content="${1}"
    if [ ${FLAG_TYPE} -eq 1 ]; then
        printf "%s" "${content}" | ${CMD_TYPE}
    else
        printf "%s" "${content}" | ${CMD_COPY}
        printf "%s" "${content}" | ${CMD_TYPE}
    fi
}
...
function output_field() {
    ...
    if [ ${FLAG_AUTOTYPE} -eq 1 ] || [ "${field}" = "autotype" ]; then
        ...
        do_output "$(pass_get "${passname}" "${field}")"
    fi
}
