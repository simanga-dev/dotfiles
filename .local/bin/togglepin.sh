...
ZenExtensionCheck=$(hyprctl activewindow -j | jq -r 'select(.class == "zen") | select(.title | test("^Extension: .*"))')

# Function to toggle floating and adjust the window size/position
toggle_floating_and_resize() {
    hyprctl dispatch togglefloating active on
...
toggle_zen_extension_popup() {
    hyprctl dispatch setfloating active
    hyprctl dispatch resizeactive exact 900 700
    hyprctl dispatch centerwindow
}

# Main logic
if [ -n "${ZenExtensionCheck}" ]; then
    if [ "${WinFloat}" = "true" ]; then
        hyprctl dispatch togglefloating active
    else
        toggle_zen_extension_popup
    fi
elif [ "${WinFloat}" = "false" ] && [ "${WinPinned}" = "false" ] && ( [ "${ActiveWindowClass}" = "mpv" ] || [ -n "${PiPCheck}" ] ); then
    toggle_floating_and_resize
